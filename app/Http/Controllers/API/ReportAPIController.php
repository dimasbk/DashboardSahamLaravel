    ...
    public function reportHistory($year)
    {
       ...
       // $year = 2023;
        ...
        $data = PortofolioBeliModel::join('tb_saham', 'tb_portofolio_beli.id_saham', '=', 'tb_saham.id_saham')
            ->select('tb_portofolio_beli.id_saham', 'tb_saham.nama_saham', DB::raw('SUM(tb_portofolio_beli.volume) AS total_volume_beli'), DB::raw('AVG(tb_portofolio_beli.harga_beli) AS avg_harga_beli'),DB::raw('SUM(tb_portofolio_beli.total_beli) AS total_beli_banget'), DB::raw('AVG(tb_portofolio_beli.total_beli) AS avg_total_beli'))
        ...
        for ($i = 0; $i < count($data); $i++) {
           ...
           ->select('tb_portofolio_jual.id_saham', 'tb_saham.nama_saham', DB::raw('SUM(tb_portofolio_jual.volume) AS total_volume_jual'), DB::raw('AVG(tb_portofolio_jual.harga_jual) AS avg_harga_jual'), DB::raw('SUM(tb_portofolio_jual.total_jual) AS total_jual_banget'),DB::raw('AVG(tb_portofolio_jual.total_jual) AS avg_total_jual'))
           ...
           ->get()->toArray();
            ...
            $hargaclose = $untung['data']['results'][0]['close'];
               ...
               if ($jualReport != null){
                ...
                foreach ($jualReport as &$item) {

                    $item["tag"] = 'jual';

                }
                $jual_total = 0;
                foreach ($jualReport as $rep) {
                    if ($rep["tag"] == "jual") {
               ...
               // $data[$i]['sisa_aset'] = (100*($data[$i]['total_volume']));
                ...
                $data[$i]['total_banget'] = ($data[$i]['avg_total_beli']-$jualReport[0]['avg_total_jual']);
                // $data[$i]['total_banget'] = ($data[$i]['avg_total_beli']-$jualReport[0]['avg_total_jual'])/((($data[$i]['avg_harga_jual']-$data[$i]['avg_harga_beli'])*$jual_total)*100);
            ...
            if (!$jualReport) {
               ...
               // $data[$i]['realisasi'] =  (((($avgJual - $avgBeli) * $jual_total))*100);
